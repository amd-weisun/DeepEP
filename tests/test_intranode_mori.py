...
import deep_ep
import mori
from deep_ep import MoriBuffer
...
def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
    ...
    for return_recv_hook in (False): # disable return_recv_hook
        for dispatch_use_fp8 in (True): # disable dispatch_use_fp8
            num_times += 1
            ...
            for zero_copy in (False): # disable zero_copy
                if zero_copy:
...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    buffer = MoriBuffer(rank,num_local_ranks, num_ranks, num_tokens, 'v1_ll')
    assert False
