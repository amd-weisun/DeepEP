            ...
            for current_x in (x_pure_rand, x, x_e4m3):
                for with_topk in (True, ): # 
                    ...
                    if gbl_num_tokens_per_expert.view(num_ranks, -1)[rank].tolist() is not recv_num_tokens_per_expert_list:
                        #print(f'[Error] Mismatch in num_tokens_per_expert on rank {rank},Expected num_tokens_per_expert shape = {gbl_num_tokens_per_expert.view(num_ranks, -1)[rank].tolist().shape}, received num_tokens_per_expert shape = {recv_num_tokens_per_expert_list.shape} :', flush=True)
