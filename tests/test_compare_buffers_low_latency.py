...
def compare_buffers(local_rank: int, num_local_ranks: int, setting: dict, run_path: str):
    ...
    def bench_once(buffer):
        ...
        # When use_fp8=True, dequantize before combine (simulating GEMM output which is BF16)
        if inputs['use_fp8']:
            combine_input = per_token_cast_back(
                packed_recv_x[0].reshape(-1, hidden),
                packed_recv_x[1].reshape(-1, hidden // 128)
            ).reshape(packed_recv_x[0].shape)
        else:
            combine_input = packed_recv_x

        combine_start.record()
        buffer.low_latency_combine(combine_input, inputs['topk_idx'], inputs['topk_weights'], handle,
                                   async_finish=False)
        combine_end.record()
        torch.cuda.synchronize()
    ...
    if run_deep:
        ...
        deep_packed_recv_x = per_token_cast_back(deep_packed_recv_x[0].reshape(-1, hidden), deep_packed_recv_x[1].reshape(-1, hidden // 128)).reshape(deep_packed_recv_x[0].shape) \
    ...
    if run_mori:
        ...
        mori_packed_recv_x = per_token_cast_back(mori_packed_recv_x[0].reshape(-1, hidden), mori_packed_recv_x[1].reshape(-1, hidden // 128)).reshape(mori_packed_recv_x[0].shape) \
